...
{
  return SignalJpsiPsi2s(x, p) + VWG(x, &p[8]);
}

// ======================================================================
// Main macro
// ======================================================================
void FitMass1Bin_VWG_Ratio(const char* filename = "/media/takuma/ESD-EAWA/Data/UPCcandMuon/0325/AnalysisResults.root")
{
  gStyle->SetOptStat(0);
  int fontCode = 42;
  ...
  // Fit
  TF1* fTotal = new TF1("fTotal", TotalFit, cfg.fitMin, cfg.fitMax, 12);
  fTotal->SetNpx(1000);
  ...
  fTotal->SetParameter(8, hUnlike->GetMaximum() * 0.6);
  // fTotal->SetParLimits(8, 0.5, hUnlike->GetMaximum() * 10.0);
  fTotal->SetParameter(9, 1.5);
  // fTotal->SetParameter(9, 0.5);
  fTotal->SetParLimits(9, 0.0, 2.5); // out of mass range
  fTotal->SetParameter(10, 0.5);     // A
  fTotal->SetParLimits(10, 0.1, 1.0);
  fTotal->SetParameter(11, 0.3); // B
  fTotal->SetParLimits(11, 0.0, 1.0);
  ...
  TF1* fBg = new TF1("fBg", VWG, cfg.fitMin, cfg.fitMax, 4);
  for (int i = 0; i < 4; i++) {
  ...
  legend->AddEntry(fBg, "Background (VWG)", "L");
