...
RETRY_LIST_DIR = os.path.join(BASE_DIR, "List_Retry0408") # 分割リストの保存先
...
def main():
    ...
    tasks_to_run = []
    failed_tasks_info = [] # 再実行用にタスク情報を保持

    # ==========================================
    # フェーズ1: 初回並列実行
    # ==========================================
    for dataset, conf_file in DATASET_MAP.items():
        list_dir = os.path.join(BASE_DIR, "List", dataset)
    ...
    with ProcessPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for future in as_completed(futures):
            original_task_info = futures[future] # (dataset, conf, list_path, basename)
    ...
    if final_failed_tasks:
        ...
        print("※これらのファイルリストには破損したROOTファイルが含まれている可能性が高いです。")
    else:
