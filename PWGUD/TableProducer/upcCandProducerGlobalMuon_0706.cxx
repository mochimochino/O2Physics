...
/// \file   upcCandProducerGlobalMuon_0706.cxx
/// \brief  UPC candidate producer for forward muons with MFT support - UNDER DEVELOPMENT
/// \note   Revision 0706: fixes 3 issues found by comparing this propagation logic against
///         skimmerPrimaryMuon.cxx / skimmerPrimaryMFTTrack.cxx (see
///         upc-analysis-run3/docs/mft-mch-mid-propagation-comparison.md):
///         (1) propagateMCHtoZ() was applying the vertex-constrained Branson correction to the
///             MFT z-plane instead of the true vertex, biasing the reconstructed slope and
///             underestimating pT -- switched to extrapToVertexWithoutBranson().
///         (2) the vertex used for Branson-corrected propagation was hardcoded to (0,0,0)
///             instead of the FwdDCAFitter secondary vertex already computed in
///             createCandidates() -- now threaded through to addToFwdTable().
///         (3) no z-shift correction existed for the MCH/MFT matching-plane alignment offset
///             (present in the other two producers) -- added as fApplyZShiftFromCCDB/fManualZShift.
...
struct UpcCandProducerGlobalMuon {
  ...
  // z-shift configurables: corrects a known MCH/MFT matching-plane alignment offset.
  // Mirrors skimmerPrimaryMuon.cxx / skimmerPrimaryMFTTrack.cxx so results are consistent
  // across the PWGEM and PWGUD forward-muon producers.
  Configurable<bool> fApplyZShiftFromCCDB{"fApplyZShiftFromCCDB", false, "flag to apply z shift from CCDB"};
  Configurable<std::string> fZShiftPath{"fZShiftPath", "Users/m/mcoquet/ZShift", "CCDB path for z shift to apply to forward tracks"};
  Configurable<float> fManualZShift{"fManualZShift", 0.f, "manual z-shift for propagation of MCH to the MFT matching plane"};
  ...
  // Propagate MCH track to the actual (fitted) vertex, applying the full Branson + energy-loss
  // correction. Unlike the previous revision, the vertex position is a real argument rather than
  // a hardcoded (0,0,0): for global (MFT-matched) candidates the caller passes the FwdDCAFitter
  // secondary vertex already computed in createCandidates() for that branch. For pure MCH-MID/MCH
  // candidates the caller still passes (0,0,0) -- a FwdDCAFitter PCA is NOT valid there since
  // those raw tracks are downstream of the absorber and the fitter ignores absorber material
  // (see the NOTE ON VERTEX comment at the MCH-MID loop in createCandidates()). Using the true
  // vertex matters here because extrapToVertex() geometrically reconstructs the track's slope
  // from a line connecting the Branson plane to (vtxX, vtxY, vtxZ) -- a wrong vertex directly
  // biases the momentum.
  auto propagateToZero(ForwardTracks::iterator const& muon, double vtxX, double vtxY, double vtxZ, double errXVtx = 0., double errYVtx = 0.)
  {
    ...
    o2::mch::TrackExtrap::extrapToVertex(mchTrack, vtxX, vtxY, vtxZ, errXVtx, errYVtx);
  ...
  // Propagate MCH track to a target z point that is NOT the vertex (typically the MFT track's z,
  // used to combine MCH momentum with MFT position/angle in refitGlobalMuonCov()).
  // IMPORTANT: this must NOT use extrapToVertex()/Branson correction here. Branson correction
  // reconstructs the track's slope from a straight line between the Branson plane and the point
  // passed in as "vertex" -- if that point is the MFT z-plane rather than the true vertex (which
  // sits further away, on the other side of the MFT), the reconstructed slope is systematically
  // too steep and the recombined pT comes out biased low (see
  // upc-analysis-run3/docs/mft-mch-mid-propagation-comparison.md, section 8). Using
  // extrapToVertexWithoutBranson() keeps the real measured position/slope (only inflating the
  // covariance for MCS, same as skimmerPrimaryMuon.cxx's kToMatchingPlane branch) and avoids
  // this bias.
  auto propagateMCHtoZ(ForwardTracks::iterator const& muon, double targetZ)
  {
    ...
    track.setCovariances(tcovs);
    auto mchTrack = fMatching.FwdtoMCH(track);
    o2::mch::TrackExtrap::extrapToVertexWithoutBranson(mchTrack, targetZ);
    auto proptrack = fMatching.MCHtoFwd(mchTrack);
  ...
  // Propagate an already MCH+MFT-combined Global track to the actual (fitted) vertex z.
  // No absorber correction is applicable here: this step stays entirely upstream of the
  // absorber (from the MFT z-plane to the vertex), exactly like skimmerPrimaryMuon.cxx's plain
  // helix step for the same z range.
  auto propagateGlobalFwdToZero(const o2::dataformats::GlobalFwdTrack& globalTrack, double vtxZ)
  {
    auto mchTrack = fMatching.FwdtoMCH(globalTrack);
    o2::mch::TrackExtrap::extrapToZCov(mchTrack, vtxZ);
    auto proptrack = fMatching.MCHtoFwd(mchTrack);
                     ...
                     double vtxX, double vtxY, double vtxZ)
  {
    ...
    if (fUpcCuts.getUseFwdCuts()) {
      ...
      if (isGlobal && fEnableMFT) {
        // Refit global muon: propagate MCH component to the MFT z-plane (no Branson: see
        // propagateMCHtoZ()), combine with MFT spatial info, then propagate to the real vertex.
        auto mchTrack = track.matchMCHTrack_as<ForwardTracks>();
        auto mfttrack = track.matchMFTTrack_as<o2::aod::MFTTracks>();
        double zMFT = mfttrack.z() + fZShift; // fZShift corrects the MCH/MFT alignment offset
        ...
        // NOTE: MFT covariance is still zeroed here (kept as in the previous revision). The
        // real per-track MFT covariance table (aod::MFTTracksCov) is joined by matchMFTTrackId,
        // not by the MFT track's own global index, and even skimmerPrimaryMuon.cxx keeps this
        // path disabled (see its processRec_TTCA_withMFTCov, commented out) pending validation
        // of that table's availability/indexing in current AO2Ds. Left as a known open item
        // rather than risk wiring a wrong table lookup here.
        SMatrix55 tcovs{};
        ...
        pft = propagateGlobalFwdToZero(refitAtMFT, vtxZ);
      } else {
        pft = propagateToZero(track, vtxX, vtxY, vtxZ);
      }
                        ...
                        const o2::aod::McFwdTrackLabels* mcFwdTrackLabels)
  {
    ...
    if (fUpcCuts.getUseFwdCuts() || fEnableMFT) {
      if (runNumber != fRun) {
        ...
        if (fEnableMFT) {
          ...
          // z-shift correction for the MCH/MFT matching-plane alignment offset (reuses the
          // run-duration timestamp already fetched above for GRPMagField, so no BCsWithTimestamps
          ...
          if (fApplyZShiftFromCCDB) {
            ...
            if (zShift != nullptr && !zShift->empty()) {
              LOG(info) << "reading z shift " << (*zShift)[0] << " from " << fZShiftPath.value;
              fZShift = (*zShift)[0];
            } else {
              LOG(info) << "z shift is not found in ccdb path " << fZShiftPath.value << ". set to 0 cm";
              fZShift = 0.f;
            }
          } else {
            LOG(info) << "z shift is manually set to " << fManualZShift.value << " cm";
    ...
    if (!mapGlobalBcsWithGlobalMuonTrackIds.empty()) {
      for (const auto& gbc_anchorids : mapGlobalBcsWithGlobalMuonTrackIds) {
        ...
        // Valid here because allTrackIds only ever holds GlobalMuonTrack/GlobalForwardTrack
        // (already MFT-matched, i.e. defined upstream of the absorber), so FwdDCAFitterN's
        // field-only propagation (no absorber material) is appropriate for finding their PCA.
        // This estimated common-origin point is then reused below as the "vertex" for the
        // Branson-corrected propagation of these same tracks (propagateToZero()/
        // propagateGlobalFwdToZero()), fixing the previous hardcoded (0,0,0) assumption.
        ...
        for (const auto& ianchor : vAnchorIds) {
          if (!addToFwdTable(candId, ianchor, globalBcAnchor, 0., fwdTracks, mftTracks, mcFwdTrackLabels, bestVtxX, bestVtxY, bestVtxZ))
            continue;
        ...
        for (const auto& [imchMft, gbc] : mapMchMftIdBc) {
          if (!addToFwdTable(candId, imchMft, gbc, (gbc - globalBcAnchor) * o2::constants::lhc::LHCBunchSpacingNS, fwdTracks, mftTracks, mcFwdTrackLabels, bestVtxX, bestVtxY, bestVtxZ))
            continue;
    ...
    // NOTE ON VERTEX: unlike the Global-track branch above, we deliberately keep the nominal
    // (0,0,0) vertex assumption here rather than computing a FwdDCAFitter PCA. These tracks
    // (MuonStandaloneTrack/MCHStandaloneTrack) are raw MCH parameters, i.e. still downstream of
    // the absorber; FwdDCAFitterN only does field-only quadratic/helix propagation with no
    // absorber material, so a PCA computed directly from these tracks would be dominated by
    // uncorrected absorber-scattering and would be a worse vertex estimate than (0,0,0), not a
    // better one. A correct fix would need an iterative Branson-correct/re-estimate-vertex
    // scheme, which is left as a follow-up (see mft-mch-mid-propagation-comparison.md).
    for (const auto& gbc_muids : mapGlobalBcsWithMCHMIDTrackIds) {
      ...
      for (const auto& imuon : vMuonIds) {
        if (!addToFwdTable(candId, imuon, globalBcMid, 0., fwdTracks, mftTracks, mcFwdTrackLabels, 0., 0., 0.))
          continue;
        numContrib++;
      ...
      for (const auto& [imch, gbc] : mapMchIdBc) {
        if (!addToFwdTable(candId, imch, gbc, (gbc - globalBcMid) * o2::constants::lhc::LHCBunchSpacingNS, fwdTracks, mftTracks, mcFwdTrackLabels, 0., 0., 0.))
          continue;
        numContrib++;
