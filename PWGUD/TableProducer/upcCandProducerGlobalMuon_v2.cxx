...
// All rights not reserved are reserved.
//
...
/// \file   upcCandProducerGlobalMuon_v2.cxx
/// \brief  UPC candidate producer for forward muons with MFT support - UNDER DEVELOPMENT
///         v2: Fixed global muon refit sequence:
///           1. Extrapolate MCH through absorber to z_MFT (MFT reference plane)
///           2. refitGlobalMuonCov at z_MFT (MCH extrap params + MFT measured params)
///           3. Propagate refitted global track from z_MFT to vertex (z=0)
...
struct UpcCandProducerGlobalMuon {
  ...
  // Propagates a MCH standalone track through the absorber to z=0 (vertex).
  // Used for non-global (MCH-MID, MCH-only) tracks.
  ...
  // Step 1 of global muon refit:
  // Extrapolates a MCH standalone track through the absorber to a target z (the MFT reference plane).
  // Unlike propagateToZero, this stops at targetZ instead of z=0.
  auto propagateMCHtoZ(ForwardTracks::iterator const& muon, double targetZ)
  {
    ...
    result.setParameters(proptrack.getParameters());
    result.setZ(proptrack.getZ());
    result.setCovariances(proptrack.getCovariances());
    return result;
  }
                     ...
                     const o2::aod::McFwdTrackLabels* mcFwdTrackLabels)
  {
    ...
    if (fUpcCuts.getUseFwdCuts()) {
      ...
      if (isGlobal && fEnableMFT) {
        // --- Correct 3-step global muon refit ---
        ...
        // Step 1: Extrapolate MCH through absorber to the MFT reference plane
        auto propMuonAtMFT = propagateMCHtoZ(mchTrack, zMFT);

        // Step 2: Refit at z_MFT — MCH extrapolated params + MFT measured params
        using SMatrix5 = ROOT::Math::SVector<double, 5>;
        using SMatrix55 = ROOT::Math::SMatrix<double, 5, 5, ROOT::Math::MatRepSym<double, 5>>;
